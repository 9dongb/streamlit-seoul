...
def predictType(total_df):
    ...
    for i in range(0, len(types)):
        ...
        # 훈련 데이터(데이터프레임) 만들기
        # 반드시 두개의 변수(컬럼) ds, y를 가지거 있어야함
        total_df2 = total_df.loc[
            total_df["HOUSE_TYPE"] == types[i], ["DEAL_YMD", "OBJ_AMT"]
        ]
        ...
        # 훈련 데이터로 학습하여 prophet 모델 만들기
        model.fit(summary_df)

        # 예측 결과를 저장할 데이터프레임 준비하기
        # 여기서는 앞으로 28일을 예측하고, 예측 데이터를 담을 데이터프레임(ds 컬럼만 존재하는) 준비
        future1 = model.make_future_dataframe(periods=periods)

        # predict() 함수를 활용하여 28일치의 데이터를 예측하기
        ...
        ax[x, y].set_title(
            f"서울시 {types[i]} 평균 가격 예측 시나리오 {periods}일간",
            fontproperties=fontprop,
        )
        ax[x, y].set_xlabel("날짜", fontproperties=fontprop)
        ax[x, y].set_ylabel("평균 가격(만원)", fontproperties=fontprop)
...
def predictDistrict(total_df):
    ...
    for sgg_nm in sgg_nms:
        ...
        fig = model.plot(forcast1, uncertainty=True, ax=ax[x, y])
        ax[x, y].set_title(
            f"서울시 {sgg_nm} 평균 가격 예측 시나리오 {periods}일간",
            fontproperties=fontprop,
        )
        ax[x, y].set_xlabel("날짜", fontproperties=fontprop)
        ax[x, y].set_ylabel("평균 가격(만원)", fontproperties=fontprop)
...
def reportMain(total_df):
    ...
    st.sidebar.download_button(
        "결과 다운로드(CSV)", csv, f"{sgg_nm}_아파트 평균 예측 {periods}일간.csv",
